...
async function syncOrderIncome(orderDoc: any, now: number) {
    ...
    const daysPassed = Math.floor(now / MS_PER_DAY) - Math.floor(lastSync / MS_PER_DAY);
    ...
    if (daysPassed > 0) {
        ...
        console.log(`Synced ${daysPassed} days of income for user ${userId}, product: ${orderData.productName}`);
    }
}
